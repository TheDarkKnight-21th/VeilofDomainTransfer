...
import time
import timm
from timm.models import load_checkpoint,load_pretrained
import tqdm
import torch
...
def get_model(model_name,path=None):

    if model_name in models.__dict__:
        # load models from common.vision.models
        backbone = models.__dict__[model_name](pretrained=True)
        if path != None: 
            print("custom_pretrain(vision)")
            checkpoint = torch.load(path)
            backbone.load_state_dict(checkpoint)
           

    # elif model_name in torch_models.__dict__:
    #     # load models from common.vision.models
        
    #     backbone = getattr(torch_models, model_name)(pretrained=True)
    #     if path != None: 
    #         print("custom_pretrain(torchvision)")
    #         checkpoint = torch.load(path)
    ...
    #         #print(backbone.features.output_features)
    #         backbone = backbone.features
        ...
        if path != None:
            ...
            print("custom_pretrain(timm)")
            backbone = timm.create_model(model_name, pretrained=True,**factory_kwargs)
        else: 
