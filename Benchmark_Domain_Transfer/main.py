...
def main(args: argparse.Namespace):
    ...
    if args.custom_pretrained != None:
        log_name = 'logs/' + args.custom_pretrained.split('/')[5] + '/' + args.log + args.arch + '_' + args.data + '_' + '_src_' + '_'.join(args.sources) + '_custom_pretrained' 
    else:
        log_name = 'logs/' + args.arch + '/' + args.log + args.data + '_' + args.arch + '_src_' + '_'.join(args.sources)
    ...
    for opt in  ['sgd', 'adam']:
        ...
        if opt == 'sgd':
            lr_list  = [1e-2] # , 1e-3, 1e-4 : 1e-2 is the best lr for the acc
        if opt == 'adam':
            lr_list  = [1e-3] # , 1e-4, 1e-5 : 1e-3 is the best lr for the acc
        ...
        for lr in lr_list:
            ...
            if args.log_wandb:
                experiment = f"UTL-21k-{args.data}"

                if args.custom_pretrained:
                    wandb_name = f"{args.custom_pretrained.split('/')[5]}_{opt}_{lr}"
                else : 
                    wandb_name = f"{args.arch}_{opt}_{lr}" 
            ...
            for seed in [0]:
                ...
                if model_name == 'dino_vits16':
                    ...
                    backbone = utils.get_model(args.arch,args.custom_pretrained)
                ...
                if args.phase != 'train':
                    checkpoint = torch.load(logger.get_checkpoint_path('global_best'), map_location='cpu')
                ...
                for epoch in range(args.epochs):
                    ...
                    if epoch % test_iter == 1:
                        ...
                        print("정띵 : ",acc1_list,test_acc)
                        
                        log = {}
                        if args.log_wandb:
                            for idx,domain_name in enumerate(args.targets):
                                log[domain_name] = acc1_list[idx]
                            log["Top1 Avg"] = test_acc
                            log["Source Acc"] = acc1
                            wandb.log(log,step = epoch)


                        logger.write(f"Epoch : {epoch}, Evaluate on test set...")
...
def train(train_iter: ForeverDataIterator, model: Classifier, optimizer,
    ...
    for i in range(args.iters_per_epoch):
        ...
        if i % args.print_freq == 0:
            ...
            rest_time =  (time.time()-start_time) * (((args.iters_per_epoch)-(i+1))/(i+1))
            total_time = ongoing_time + rest_time
            # 초 단위 시간을 다른 단위로 변환
            days1,hours1,minutes1,seconds1 = make_time(rest_time)
            days2,hours2,minutes2,seconds2 = make_time(total_time)
            print(f"{hours1}h:{minutes1}m:{seconds1}s / {hours2}h:{minutes2}m:{seconds2}s")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu-id', default="0", type=str,
                        help='gpu id to run')
    parser.add_argument('--custom-pretrained', default=None, type = str,
                        help='local pretrained directory')
