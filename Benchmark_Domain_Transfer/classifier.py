...
class Classifier(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck: Optional[nn.Module] = None,
        ...
        self.ex = nn.AdaptiveAvgPool2d(output_size=(1))
        ...
        if bottleneck is None:
            ...
            self._features_dim = backbone.out_features # 768 convnext_tiny output
        else:
...
class ImageClassifier(Classifier):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # print(f.shape)        
        # print("1",f.shape)
        # code for dimenstion issue
        if (f.shape[1] != self._features_dim) :
             target_idx = f.shape.index(self._features_dim)
             rest_indices = [i for i in range(len(f.shape)) if i not in [0, target_idx]]
             f = f.permute(0,target_idx,*rest_indices) 

             # f.shape = [batch,channel,spatial info] , self.pool_layer(f).shape => [batch,1] :( / so using unsqueeze for pooling  
             if len(f.shape) == 3:
                 f = f.unsqueeze(-1)
        # print("2",f.shape,self.ex(f).shape)
        

        if len(f.shape) > 2:
            f = self.pool_layer(f)
        # print("3",f.shape)
        # print("header input dim : ",self._features_dim)
