...
Train loss (grid search 中)
<img src="jm_diary/loss_wandb.png" width='830px'/>


Data pruning with wordnet
데이터 pruning(가지치기)을 위해서는 경로 변경 후, 아래 명령어를 실행해주세요.
```
python data_pruning.py --down-dir DOWNSTREAM DATASET DIR --up-dir IN22k DATASET DIR --threshold THRESHOLD
```

Train hyperparameter
사전학습을 위해서는 경로 변경 후, 아래 명령어를 실행해주세요.
```
bash ./pretrain.sh
```
...
```
{args.model}_{args.lr}_{args.warmup_lr}_{args.warmup_epochs}_{args.batch_size}x{torch.cuda.device_count()}
```
